...
final class SnackListPresenter {
    ...
    func setupUserAccount(_ account: UserEntity) {
        self.view?.userAccount = account
    }
    ...
    private func makeSectionsItemsFromSnacks(snackList: [SnackModel]) -> [SnackByDate] {
        ...
        let sectionDates = snackList.map({ dateFormatter.string(from: $0.date) })
        let sortedSectionDates = Set(sectionDates).sorted(by: <)
